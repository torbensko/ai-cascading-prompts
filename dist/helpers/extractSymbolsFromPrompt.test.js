...
describe("extractSymbolsFromPrompt", () => {
    it("extracts symbols enclosed in asterisks and returns a cleaned prompt", () => {
        ...
        const { symbols, cleanedPrompt } = (0, extractSymbolsFromPrompt_1.extractSymbolsFromPrompt)(prompt);
        expect(symbols).toEqual(["test", "multiple"]);
        expect(cleanedPrompt).toBe("This is a test prompt with multiple symbols.");
    });
    ...
    it("handles symbols at the start and end of the prompt", () => {
        ...
        expect(cleanedPrompt).toBe("start and end with symbols");
    });
